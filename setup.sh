...
sudo chmod 700 /etc/datadog-agent/
sudo chown dd-agent. /home/vagrant/data/mysql_password.py 
...
sudo mysql --user='root' --execute="CREATE USER datadog@'%' IDENTIFIED WITH mysql_native_password by 'Datadog2023'; ALTER USER datadog@'%' WITH MAX_USER_CONNECTIONS 5; GRANT REPLICATION CLIENT ON *.* TO datadog@'%';GRANT PROCESS ON *.* TO datadog@'%'; FLUSH PRIVILEGES;"
